"""
Script to create and run all the VAEs as described in the paper.
...
status0_data = pd.read_table("ADNI0_cc_status.txt", header=None, sep=' ')
...
all_df_before = pd.concat(all_pandas)
...
for (cur_status, all_df) in [('ad', all_df_ad), ('ctr', all_df_ctr)]:
    ...
    for step in range(75):
        ...
        gene_input = Input(shape=(original_dim,))
        ...
        # Uncomment for some information printed in the terminal for each VAE created
        # print("########################################")
        # print("########################################")
        # print(latent_dim)
        # print(batch_size)
        ...
        # Saving evolution of training/test losses to check overfitting
        file_dir = os.path.join('multiple_vaes',
                                "hist_" + cur_status + '_' + str(latent_dim) + '_' + str(step) + '_' + str(
                                    batch_size) + '_' + str(epochs) + '_' + str(learning_rate) + '_' + str(
                                    kappa) + '.tsv')
        history_df.to_csv(file_dir, sep='\t')
        ...
        encoder_model_file = os.path.join('multiple_vaes',
                                          'models_' + cur_status + '_' + str(latent_dim) + '_' + str(step) + '_' + str(
                                              batch_size) + '_' + str(epochs) + '_' + str(learning_rate) + '_' + str(
                                              kappa) + '_encoder_onehidden_vae.hdf5')
        decoder_model_file = os.path.join('multiple_vaes',
                                          'models_' + cur_status + '_' + str(latent_dim) + '_' + str(step) + '_' + str(
                                              batch_size) + '_' + str(epochs) + '_' + str(learning_rate) + '_' + str(
                                              kappa) + '_decoder_onehidden_vae.hdf5')
